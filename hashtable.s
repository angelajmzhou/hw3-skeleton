...
insertData:
	...
	lw s4 0(s0)
	jalr ra 0(s4)	#run hashFunction on key

	mv t5 a0		#t5 = hashFunction(key)
	lw t1 12(s0) 	#t1 = table -> size
	rem t5 t5 t1	#t5 = ((table->hashFunction)(key)) % table->size;
	...
	lw t5 16(s0)		#t5 = used
	addi t5 t5 1	#t5+=1
	sw t5 16(s0)		#increment and re-store the "used" variable
...
findData:
	...
	mv a0 s1		#set up parameters (key)
	lw t1 0(s0)		#get hashFunction pointer
	jalr ra 0(t1)	#hashFunction(key)
	#mv s3 a0
	lw t0 12(s0)	#t0 = table -> size
	rem t1 a0 t0	#t1 (location)= ((table->hashFunction)(key)) % table->size
	

	lw t2 8(s0)		#table->data ~get the pointer
	slli t1 t1 2	#location = location*4
	add t2 t2 t1	#get address of data[location]
...
while_start:
	...
	lw t0 4(s0)
	jalr ra 0(t0)	#equalFunction(key, lookAt->key)
	mv s3 a0
	beqz s3 continue
	lw a0, 4(s2)
	j epilogue
